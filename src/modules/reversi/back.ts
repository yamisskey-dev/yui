...
class Session {
	 ...
	 * 
	 * TODO: 改善提案
	 * - 接待時はまるっと処理の中身を変え、とにかく相手が隅を取っていること優先な評価にする
	 * - 接待のときは、どちらかというと「自分が不利になる手を選ぶ」というよりは、「相手に角を取らせられる手を選ぶ」ように思考する
	 * - 残りターン数というよりも「空いているマスが12以下」の場合に完全読みさせる
	 * - cansをまず浅く読んで(または価値マップを利用して)から有益そうな手から順に並べ替え、効率よく枝刈りできるようにする
	 * - ここに (隅以外の確定石の数 * 100) をスコアに加算する処理を入れる
	 */
	private staticEval = () => {
	...
	private think = () => {
		...
		// 改善提案: 接待のときは、どちらかというと「自分が不利になる手を選ぶ」というよりは、「相手に角を取らせられる手を選ぶ」ように思考する
		...
		const dive = (pos: number, alpha = -Infinity, beta = Infinity, depth = 0): number => {
			...
			if (depth === maxDepth) {
				...
				// 改善提案: 残りターン数というよりも「空いているマスが12以下」の場合に完全読みさせる
				const nextDepth = (this.strength >= 4) && ((this.maxTurn - this.currentTurn) <= 12) ? Infinity : depth + 1;

				// 次のターンのプレイヤーにとって最も良い手を取得
				// 改善提案: cansをまず浅く読んで(または価値マップを利用して)から有益そうな手から順に並べ替え、効率よく枝刈りできるようにする
				for (const p of cans) {
					if (isBotTurn) {
